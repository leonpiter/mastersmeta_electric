...
export function symbolToSvg(sym: SymbolDef, opts: SymbolRenderOpts = {}): SVGGElement {
  ...
  for (const prim of sym.graphics) {
    switch (prim.type) {
      case "line":
        g.append(el("line", {
          x1: prim.x1, y1: prim.y1, x2: prim.x2, y2: prim.y2,
          stroke, "stroke-width": sw, "stroke-linecap": "round",
        }));
        break;
      case "rect":
        g.append(el("rect", {
          x: prim.x, y: prim.y, width: prim.w, height: prim.h,
          fill: "none", stroke, "stroke-width": sw,
        }));
        break;
      case "circle":
        g.append(el("circle", {
          cx: prim.cx, cy: prim.cy, r: prim.r,
          fill: "none", stroke, "stroke-width": sw,
        }));
        break;
      case "text": {
        const t = el("text", {
          x: prim.x, y: prim.y,
          "font-size": prim.size ?? 3, fill: stroke,
