...
export interface SymbolInstance {
  ...
  labelFields?: string[];
  /**
   * Метка сигнала соединителя страниц (S29). Соединители (`kind: "page-connector"`)
   * с одинаковой меткой электрически связаны (одна цепь через листы) и адресуют друг друга.
   */
  signal?: string;
  /**
   * Сдвиг таблицы «зеркало контактов» относительно катушки (мм), чтобы не мешала подписям.
   * Только у master-катушки; перетаскивается мышью. Пусто = под катушкой по умолчанию.
   */
  mirrorDx?: number;
  mirrorDy?: number;
}
